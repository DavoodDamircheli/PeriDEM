...
gen_setup(){
    ...
    python3 $path/setup.py --path "$dir" $hopper_ratio $particle_rad $nx $ny $L $wallh_ratio $vel $acc $rho_scale $K_scale $G_scale $mshfac $delta_factor $contact_rad_factor $plot >> $logfile
    cp setup.png $dir/
}
