...
# --- compute characteristic node spacing h from all particles ---
def mean_nn_spacing(particles, sample_per_particle=200):
    import numpy as np
    ...
    for row in particles:
        P = np.asarray(row[0].pos, dtype=np.float64)  # (Ni,3)
        if P.shape[0] < 3:
            continue
        # Optional downsample for speed
        idx = np.arange(P.shape[0])
        if P.shape[0] > sample_per_particle:
            idx = rng.choice(P.shape[0], size=sample_per_particle, replace=False)
        Q = P[idx]

        # brute-force NN (OK for samples); swap in a KDTree if large
        for q in Q:
            d = np.linalg.norm(P - q, axis=1)
            d[d==0] = np.inf
            d_list.append(d.min())
    return float(np.mean(d_list)), float(np.median(d_list))
...
def main():
    ...
    ap.add_argument("--N", type=int, default=1, help="Number of particles to use.")
    ...
    ap.add_argument("--nz", type=float, default=4.0, help="mm")
    ...
    # After particles are generated (so we can measure h):
    h_mean, h_med = mean_nn_spacing(particles)
    # Choose scale factors (tweakable via CLI if you like)
    alpha = 3.0   # delta ~ 3 h  (2–4 is typical)
    beta  = 1.5   # rc   ~ 1.5 h (1–2 is typical)
    ...
    #wall = Wall3d(1, -x_max/2, -y_max/2, -z_max/2, x_max/2, y_max/2,z_max/2) 
