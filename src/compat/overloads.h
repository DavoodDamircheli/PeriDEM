...
template <typename ttype> ostream &operator<<(ostream &o, vector<ttype> v) {
  size_t N = v.size();
...
// Addition of vector of objects, isn't it obvious? Now we need to define
// _every_ type of point wise operator :(
...
template <typename ttype> vector<ttype> operator*(double s, vector<ttype> v1) {
  unsigned N1 = v1.size();
  vector<ttype> v3(N1);
