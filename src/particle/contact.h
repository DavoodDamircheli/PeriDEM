...
// #include "particle/particle2.h" //#include "particle/nbdarr.h" #include
// "compat/overloads.h"
#include "particle/particle3.h" //#include "particle/nbdarr.h" #include "compat/overloads.h"
using namespace std;
...
Matrix<double, 1, dim> vortexfield(Matrix<double, 1, dim> pos, bool normalized,
  ...
  if (0.5 * sin(M_PI * float(time) / freq) >=
      0) // sinusoidal function to regulate current fluctuation using an
         // arbitrary frequency
  {
...
template <unsigned dim>
Matrix<double, 1, dim> shearfield(Matrix<double, 1, dim> pos, bool normalized,
                                  double scaling, double angle, unsigned time) {
  Matrix<double, 1, dim> v;
  v(0) = scaling * pos(1);
  v(1) = 0;
  return v;
...
public:
  ...
  void apply_config(ConfigVal CFGV) {
    ...
    if (dim == 2) {
      ...
      if (CFGV.wall_bottom != (-999)) {
        bottom = CFGV.wall_bottom;
      }
      ...
      if (CFGV.wall_x_min != (-999)) {
        x_min = CFGV.wall_x_min;
      }
      if (CFGV.wall_y_min != (-999)) {
        y_min = CFGV.wall_y_min;
      }
      if (CFGV.wall_z_min != (-999)) {
        z_min = CFGV.wall_z_min;
      }
      if (CFGV.wall_x_max != (-999)) {
        x_max = CFGV.wall_x_max;
      }
      if (CFGV.wall_y_max != (-999)) {
        y_max = CFGV.wall_y_max;
      }
      if (CFGV.wall_z_max != (-999)) {
        z_max = CFGV.wall_z_max;
      }
...
// &f_toupdate_i)
// wall_cyl_side_force_update<dim>(dist_from_cyl_wall, CN, cyl_inward_normal_x,
// cyl_inward_normal_y, P_C.vel[i], P_C.vol[i], P_C.rho,
// giant_f[i+i_starting_index], dt, Wall.speed_r, Wall.reaction_r);
void wall_cyl_side_force_update(
    double dist, Contact CN, double cyl_inward_normal_x,
    double cyl_inward_normal_y, Matrix<double, 1, dim> vel_i, double vol_i,
    double rho, Matrix<double, 1, dim> &f_toupdate_i, double dt,
    double wall_side_speed, Matrix<double, 1, dim> &wall_reaction_force) {
  ...
  if (abs(dist) <= CN.contact_rad) {
    ...
    // Here, this_contact_force = K_n(R_c - |d(x)|) V(w). So, we just multiply
    // V(x)
    wall_reaction_force(0) -=
        (vol_i * c_sign * this_contact_force * cyl_inward_normal_x);
    wall_reaction_force(1) -=
        (vol_i * c_sign * this_contact_force * cyl_inward_normal_y);

    // compute the relative velocity from the speed of wall boundary
    auto rel_vel_i = vel_i;
    // rel_vel_i[dir] -= Wall.wall_side_speed;

    double rel_vel_normal_proj =
        rel_vel_i[0] * cyl_inward_normal_x + rel_vel_i[1] * cyl_inward_normal_y;
    ...
    if (CN.allow_friction) {
      ...
      if (dim == 2) {
        ...
        //   double velocity_tangential_sign = (rel_vel_i(dir_tang) > 0 ? 1 :
        //   -1);
        //   // Debug
        ...
        //
        // // projection of velocity to the tangential direction (i.e. normal to
        // wall) auto vel_proj_1 = rel_vel_i(dir_tang_1); auto vel_proj_2 =
        // rel_vel_i(dir_tang_2);
        //
        ...
        auto vel_tang_proj =
            rel_vel_i - rel_vel_normal_proj * unit_tangential_dir;
        double vel_proj_norm =
            sqrt(pow(vel_tang_proj(0), 2) + pow(vel_tang_proj(1), 2) +
                 pow(vel_tang_proj(2), 2));

        // What if the norm is zero?
        if (vel_proj_norm > 0) {
          auto unit_comp = vel_tang_proj / vel_proj_norm;
          Matrix<double, 1, dim> friction_force =
              (-CN.friction_coefficient) * abs(this_contact_force) * unit_comp;
...
// &f_toupdate_i)
void wall_side_force_update(double dist, Contact CN, unsigned dir,
                            Matrix<double, 1, dim> vel_i, double vol_i,
                            double rho, Matrix<double, 1, dim> &f_toupdate_i,
                            double dt, double wall_side_speed,
                            Matrix<double, 1, dim> &wall_reaction_force) {
...
// force on the central particle by the neighboring particle
template <unsigned dim>
void update_contact_force_by_boundary(ParticleN<dim> P_C, ParticleN<dim> P_N,
                                      Contact CN, double dt,
                                      vector<Matrix<double, 1, dim>> &giant_f,
                                      unsigned i_starting_ind,
                                      unsigned j_starting_ind,
  ...
  for (unsigned p = 0; p < tn_i; ++p) {
    ...
    for (unsigned q = 0; q < tn_j; ++q) {
      ...
      if (dir_norm <= CN.contact_rad) {
        ...
        // assuming that normal_stiffness = material.cnot(delta=contact_rad) /
        // contact_rad
        ...
        if (is_self_contact) {
          if (i == j) {
            do_compute = 0;
          } else if (std::find(P_C.NbdArr[i].begin(), P_C.NbdArr[i].end(), j) !=
                     P_C.NbdArr[i].end()) {
            ...
            // Otherwise (R_c >= delta), long-range repulsion acts between far
            // away nodes  (delta<|xi|<R_c) whereas no repulsion between
            // short-range connected bonds (|xi|<delta), which is inconsistent
            // and will lead to instability
            len_ref = xi_ij_norm;
        ...
        if (do_compute &&
            (dir_norm < len_ref)) { // compression only: dir_norm < len_ref
          ...
          // moved this logic to if; repulsive-only: only when s is negative,
          // repulsive force acts on node i if (str > 0) {
          //     str = 0;
          // }
          ...
          if (!is_self_contact) { // for self-contact, equal and opposite force
                                  // is redundant
          ...
          //     //// Cohesion
          //
          //     //double cohesion_dist_contrib = (CN.contact_rad - 2 * dir_norm
          //     * CN.cohesion_scaling);
          //     //
          //     // reflection of repulsion curve
          //     //double cohesion_dist_contrib =  -dir_norm *
          //     CN.cohesion_scaling;
          //     // constant
          //     double cohesion_dist_contrib =  -CN.cohesion_scaling *
          //     spring_const;
          //     // auto f_cohesion = (-K_n * cohesion_dist_contrib * N_vol) *
          //     unit_dir; auto f_cohesion = (-spring_const *
          //     cohesion_dist_contrib * N_vol) * unit_dir;
          ...
          if (CN.allow_damping) {
            ...
            // Did not work, probably because the idea is incorrect (that the
            // velocity should be zero at the next timestep if the sign changes
            // from this to the next timestep)
            // if ( (P_C.rho/dt * rel_v_normal_comp.norm()) <
            // (contact_force_norm + damping_coefficient *
            // rel_v_normal_comp.norm()) ) {
            ////std::cout << "Edit here" << std::endl;
            // double f_damping_abs = P_C.rho / dt * rel_v_normal_comp.norm() -
            // contact_force_norm;

            // f_damping = -f_damping_abs / rel_v_normal_comp.norm() *
            // rel_v_normal_comp;
            //}
            ...
            if (!is_self_contact) { // for self-contact, equal and opposite
                                    // force is redundant
              if (P_N.stoppable) {
                // giant_f[j + j_starting_ind] -= f_damping;
                giant_f[j + j_starting_ind] -=
                    f_damping / sqrt(N_vol) * sqrt(P_C.vol[i]);
              }
            }
          }
          ...
          if (CN.allow_friction) {
            ...
            // v - (v.e)e, relative velocity of center in the tangential
            // direction
            auto rel_v_tangential_comp = rel_v - rel_v.dot(unit_dir) * unit_dir;
            ...
            if (rel_v_tangential_norm > 0) {
              ...
              f_friction = -CN.friction_coefficient * contact_force_norm *
                           rel_v_tangential_unit;
              // Correction for small velocity
              if (rel_v_tangential_norm <
                  CN.friction_coefficient * contact_force_norm * dt / P_C.rho) {
                // std::cout << "small velocity detected" << std::endl;
                f_friction = -rel_v_tangential_norm / P_C.rho / dt *
                             rel_v_tangential_unit;
              }
              // combined_contact_force[i] += f_friction;
              if (P_C.stoppable) {
...
template <unsigned dim>
void apply_force_field(ParticleN<dim> P_C, int forcefield_type, double dt,
                       unsigned t, unsigned timesteps,
                       vector<Matrix<double, 1, dim>> &giant_f,
                       unsigned i_starting_ind, double forcefield_scaling,
                       bool normalized, bool by_boundary) {
  // if specified, work with all the nodes, otherwise work with outer nodes
  ...
  for (unsigned p = 0; p < tn_i; p++) {
    ...
    if (forcefield_type == 1) {
      ...
      // combined_contact_force[i] += vortexfield<dim>(P_C.CurrPos[i],
      // normalized, forcefield_scaling, -M_PI, t);
      giant_f[i + i_starting_ind] += vortexfield<dim>(
          P_C.CurrPos[i], normalized, forcefield_scaling, -M_PI, t);
    } else if (forcefield_type == 2) {
      // shear field
      // combined_contact_force[i] += shearfield<dim>(P_C.CurrPos[i],
      // normalized, forcefield_scaling, angle, t);
      giant_f[i + i_starting_ind] += shearfield<dim>(
          P_C.CurrPos[i], normalized, forcefield_scaling, angle, t);
    }
  }
};
...
void update_wall_contact_force_by_boundary(
  ...
  for (unsigned p = 0; p < tn_i; p++) {
    ...
    if (dim == 2) {
      ...
      // single side interaction
      // Debug: added TL
      wall_side_force_update<dim>(y_dist_from_bottom, CN, 1, P_C.vel[i],
                                  P_C.vol[i], P_C.rho,
                                  giant_f[i + i_starting_index], dt,
                                  Wall.speed_bottom, Wall.reaction_bottom);
      wall_side_force_update<dim>(x_dist_from_left, CN, 0, P_C.vel[i],
                                  P_C.vol[i], P_C.rho,
                                  giant_f[i + i_starting_index], dt,
                                  Wall.speed_left, Wall.reaction_left);
      wall_side_force_update<dim>(x_dist_from_right, CN, 0, P_C.vel[i],
                                  P_C.vol[i], P_C.rho,
                                  giant_f[i + i_starting_index], dt,
                                  Wall.speed_right, Wall.reaction_right);
      wall_side_force_update<dim>(
          y_dist_from_top, CN, 1, P_C.vel[i], P_C.vol[i], P_C.rho,
      ...
      wall_side_force_update<dim>(dist_from_z_min, CN, 2, P_C.vel[i],
                                  P_C.vol[i], P_C.rho,
                                  giant_f[i + i_starting_index], dt,
                                  Wall.speed_z_min, Wall.reaction_z_min);
      wall_side_force_update<dim>(dist_from_z_max, CN, 2, P_C.vel[i],
                                  P_C.vol[i], P_C.rho,
                                  giant_f[i + i_starting_index], dt,
                                  Wall.speed_z_max, Wall.reaction_z_max);
      ...
      if (Wall.wall_cyl) {
        ...
        double dist_from_cyl_cent = pow(
            pow(dist_from_cyl_cent_x, 2) + pow(dist_from_cyl_cent_y, 2), 0.5);
        double dist_from_cyl_wall = dist_from_cyl_cent - Wall.wall_cyl_rad;

        double cyl_inward_normal_x =
            (dist_from_cyl_cent > 0 ? dist_from_cyl_cent_x / dist_from_cyl_cent
                                    : 0);
        double cyl_inward_normal_y =
            (dist_from_cyl_cent > 0 ? dist_from_cyl_cent_y / dist_from_cyl_cent
                                    : 0);

        // void wall_side_force_update(double dist, Contact CN, unsigned dir,
        //                             Matrix<double, 1, dim> vel_i, double
        //                             vol_i, double rho, Matrix<double, 1, dim>
        //                             &f_toupdate_i, double dt, double
        //                             wall_side_speed, Matrix<double, 1, dim>
        //                             &wall_reaction_force) {
        wall_cyl_side_force_update<dim>(
            dist_from_cyl_wall, CN, cyl_inward_normal_x, cyl_inward_normal_y,
            P_C.vel[i], P_C.vol[i], P_C.rho, giant_f[i + i_starting_index], dt,
            0.0, Wall.reaction_r);

      } else {
        // single side interaction
        wall_side_force_update<dim>(dist_from_x_min, CN, 0, P_C.vel[i],
                                    P_C.vol[i], P_C.rho,
                                    giant_f[i + i_starting_index], dt,
                                    Wall.speed_x_min, Wall.reaction_x_min);
        wall_side_force_update<dim>(dist_from_x_max, CN, 0, P_C.vel[i],
                                    P_C.vol[i], P_C.rho,
                                    giant_f[i + i_starting_index], dt,
                                    Wall.speed_x_max, Wall.reaction_x_max);

        wall_side_force_update<dim>(dist_from_y_min, CN, 1, P_C.vel[i],
                                    P_C.vol[i], P_C.rho,
                                    giant_f[i + i_starting_index], dt,
                                    Wall.speed_y_min, Wall.reaction_y_min);
        wall_side_force_update<dim>(dist_from_y_max, CN, 1, P_C.vel[i],
                                    P_C.vol[i], P_C.rho,
                                    giant_f[i + i_starting_index], dt,
                                    Wall.speed_y_max, Wall.reaction_y_max);
      }
    }
  }
  // return combined_contact_force;
...
bool intersects_box_fext(vector<double> A_lrbt, vector<double> B_lrbt,
  ...
  if (dim == 2) {
    ...
    out =
        ((A_min_x <= B_max_x) && (A_max_x >= B_min_x) && (A_max_y >= B_min_y) &&
         (A_min_y <= B_max_y) && (A_min_z <= B_max_z) && (A_max_z >= B_min_z));
  }
