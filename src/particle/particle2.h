  ...
  void remove_bonds() {
    if (break_bonds) {
      ...
      for (unsigned i = 0; i < nnodes; ++i) {
        ...
        //
        // pointers that go in parallel with j, but point to memory locations
        // within other vectors
        ...
        for (auto j = stretch[i].begin(); j != stretch[i].end(); ++j) {
          ...
          if (*j > snot) {
            // Break with both positive and negative stretch, produces
            // unrealistic output, e.g. for crack_prenotch() experiment if
            // (abs(*j) > snot) {
