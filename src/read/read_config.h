...
public:
  ...
  double speed_wall_x_min, speed_wall_y_min, speed_wall_z_min, speed_wall_x_max,
      speed_wall_y_max, speed_wall_z_max;
  ...
  // override
  // ---------------- Particle overrides (applied after loading setup.h5)
  // ----------------
  bool override_particle_vel = 0;      // 0/1
  ...
  ConfigVal() {
    ...
    override_particle_acc = 0;
    override_particle_vel = 0;
    override_particle_extforce = 0;
    add_particle_acc = 0;
    override_particle_id = -1;
    particle_acc_value = "0,0,0";
    particle_extforce_value = "0,0,0";
    particle_vel_value = "0,0,0";
  };

  // Read values from a file: name = value format, (whitespace is removed), #
  // for comment
  void read_file(std::string filename) {
    ...
    if (cFile.is_open()) {
      ...
      while (getline(cFile, line)) {
        ...
        if (value == "") {
          std::cout << "[Error] Missing value for option: " << name
                    << std::endl;
        }
        ...
        else if (name == "override_particle_extforce") {
          override_particle_acc = std::stoi(value);
        } else if (name == "override_particle_acc") {
          override_particle_acc = std::stoi(value);
        } else if (name == "override_particle_vel") {
          override_particle_vel = std::stoi(value);
        } else if (name == "add_particle_acc") {
          add_particle_acc = std::stoi(value);
        } else if (name == "override_particle_id") {
          override_particle_id = std::stoi(value);
        } else if (name == "particle_acc_value") {
          particle_acc_value = value; // e.g. "0,0,-10000"
        } else if (name == "particle_vel_value") {
